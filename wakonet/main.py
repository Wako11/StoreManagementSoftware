...
conn = sqlite3.connect("D:\wakonet\Database\store.db")
c = conn.cursor()
...
class Application:
    def __init__(self,master, *args, **kwargs):
        ...
        self.master = master
        #frames
        self.left = Frame(master, width=700, height=768, bg='skyblue')
        ...
        # components
        self.heading = Label(self.left, text = "Machakos Black Market", font=('arial 40 bold'), bg='yellow' )
        self.heading.place(x=0, y=0)

        self.date_l = Label(self.right, text="Today's Date: " + str(date), font=('arial 12 bold'), bg='lightblue', fg='white')
        ...
        self.search_btn.place(x=385, y = 120)
        ...
        self.productPrice = Label(self.left, text="", font=('arial 27 bold'), bg='white', fg='steelblue')
        self.productPrice.place(x=0, y=300)
    ...
    def ajax(self, *args, **kwargs):
        ...
        # get the prodcuts info with that id and fill it in the labels above
        ...
        self.productPrice.configure(text="Price: KSh. " + str(self.get_price))
        ...
        self.bill_btn = Button(self.left, text="Generate Bill", width=70, height=2, bg='red', fg='white', command=self.generate_bill)
        self.bill_btn.place(x=0, y = 640)

    def add_to_cart(self, *args, **kwargs):
        #get the quantity values and from the database
        ...
        if self.quantity_value > int(self.get_stock):
            ...
            for self.p in products_list:
                ...
                self.productPrice.configure(text="")
    ...
    def generate_bill(self, *args, **kwargs):
        ...
        directory = "D:\wakonet\Invoice" + str(date) + "/" 
        if not os.path.exists(directory):
            os.makedirs(directory)

            # TENPLATES FOR THE BILL
            company = "\t\t\t\tWakonet Inc. L\n"
            motor = "\t\t\t\tWe Are Limitless\n"
            address = "\t\t\t\tMachakos University\n"
            phone = "\t\t\t\t0794487104\n"
            sample = "\t\t\t\t Sample\n"
            dt = "\t\t\t\t\t" + str(date)
            ...
            for t in products_list:
                f.write("\n\t\t\t" + str(i) + "\t" + str(products_list[i] + ".........")[:7] + "\t\t" + str(product_quantity[i]) + "\t\t" + str(product_price[i]))
                i += 1
                r += 1 
